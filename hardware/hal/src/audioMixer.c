...
// Temporary dummy uses to suppress unused variable warnings REMOVE later
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	for(int i=0; i<MAX_SOUND_BITES; i++) {
		...
		if(soundBites[i].pSound == NULL) {
			...
			soundBites[i].location = 0; // Start from start of audio
			foundSpot = true;
			break;
		}
	}
	...
	if(!foundSpot) {
		printf("ERROR: No free slot was available for the new sound, try again later.\n");
	}
}
...
static void fillPlaybackBuffer(short *buff, int size)
{
	...
	// Wipe the buff to all 0's to clear any previous PCM data.
    memset(buff, 0, size * sizeof(short));

	//Since this is called from a background thread lock it
    pthread_mutex_lock(&audioMutex);
    for(int i=0; i < MAX_SOUND_BITES; i++) {
		// Found a valid sound clip
        if (soundBites[i].pSound != NULL) {
            wavedata_t *pSoundCurr = soundBites[i].pSound;
            int location = soundBites[i].location;

			// Where to start on sounds clip
            int numOfSamples = pSoundCurr->numSamples - location;
            ...
            for(int j=0; j < numOfSamples; j++) {
                int mixedSample = buff[j] + pSoundCurr->pData[location + j];

				// Handle sound clipping
			...
			// Shift location of sound
            soundBites[i].location += numOfSamples;
            if (soundBites[i].location >= pSoundCurr->numSamples) {
                soundBites[i].pSound = NULL; // Free slot, sound is done
            }
        }
    }
    pthread_mutex_unlock(&audioMutex);
